...
BN.prototype._ishlnop = function _ishlnop(op, num, shift) {
  assert(!num.sign);
  if (this.sign) {
    this.sign = false;
    this._ishlnop(op === 'add' ? 'sub' : 'add', num, shift);
    this.sign = !this.sign;
    return this;
  }

  // Bigger storage is needed
  var len = num.length + shift + (op === 'add' ? 1 : 0);
  ...
  for (var i = 0; i < num.length; i++) {
    var w = this.words[i + shift] + num.words[i] * mul + carry;
    carry = w >> 26;
...
BN.prototype._wordDiv = function _wordDiv(num, mode) {
  ...
  var diff = a.clone()._ishlnop('sub', b, m);
  if (!diff.sign) {
    a = diff;
    q.words[m] = 1;
  }
  ...
  for (var j = m - 1; j >= 0; j--) {
    ...
    qj = Math.min(qj / bhi, 0x3ffffff) | 0;
    q.words[j] = qj;

    a._ishlnop('sub', b.clone().imuln(qj), j);
    while (a.sign) {
      q.words[j]--;
      a._ishlnop('add', b, j);
    }
