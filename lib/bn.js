...
BN.prototype.divmod = function divmod(num) {
  assert(num.cmpn(0) !== 0);

  if (this.sign && !num.sign) {
    var res = this.neg().divmod(num);
    return {
      div: res.div.neg(),
      mod: res.mod.cmpn(0) === 0 ? res.mod : num.sub(res.mod)
    };
  } else if (!this.sign && num.sign) {
    var res = this.divmod(num.neg());
    return { div: res.div.neg(), mod: res.mod };
  } else if (this.sign && num.sign) {
    return this.neg().divmod(num.neg());
  }
...
BN.prototype.div = function div(num) {
  return this.divmod(num).div;
};

// Find `this` % `num`
BN.prototype.mod = function mod(num) {
  return this.divmod(num).mod;
};
