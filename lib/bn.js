...
BN.random = function random(min, max) {
  var range = max.sub(min);
  var out = new BN(null);
  out.length = range.length;
  out.words = [];

  for (var i = range.length - 1, restricted = true; i >= 0; i--) {
    var top = restricted ? range.words[i] : 1e7;
    var word = Math.round(Math.random() * top);
    out.words.unshift(word);
    if (word < top)
      restricted = false;
  }
