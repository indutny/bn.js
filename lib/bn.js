...
function jumboMulTo(self, num, out) {
  out.negative = num.negative ^ self.negative;
  out.length = self.length + num.length;
  out.words = (new FFTM(self, num)).mulp();
  return out.strip();
}
...
FFTM.prototype.convert13b = function convert13b(ws, rws, N) {
  for (var i = 0; i < N / 2; i++) {
    var w = ws[i] | 0;
...
FFTM.prototype.mulp = function mulp() {
  ...
  // Force operands to be stripped of dangling
  // useless segments prior to tweedling with their bits
  var x = this.x.strip();
  var y = this.y.strip();
...
BN.prototype.mulf = function mulf(num) {
  var out = new BN(null);
  return this.jumboMulToF(this, num, out);
};
