...
BN.prototype._init = function init(number, base) {
  ...
  if (base === 'hex')
    base = 16;
  assert(base === Math.floor(base) && base >= 2 && base <= 36);
...
BN.prototype.toString = function toString(base) {
  ...
  if (base === 16 || base === 'hex') {
    ...
    if (this.sign)
      out = '-' + out;
    return out;
  } else if (base === Math.floor(base) && base >= 2 && base <= 36) {
    var groupSize = groupSizes[base]; // Math.floor(BN.wordSize * Math.LN2 / Math.log(base));
    var groupBase = groupBases[base]; // Math.pow(base, groupSize);
    var out = '';
    var c = this.clone();
    c.sign = false;
    while (c.cmpn(0) !== 0) {
